...
namespace Proxy.Client.Utilities
{
    ...
    internal static class ResponseBuilderHelper
    {
        ...
        public static ProxyResponse BuildProxyResponse(string response, byte[] Byte, Uri destinationUri)
        {
            ...
            if (splitResponse.Length == 1)
            {
                ...
                return ProxyResponse.Create(status, Enumerable.Empty<ProxyHeader>(), Byte, Enumerable.Empty<Cookie>(), splitResponse[0]);
            }
            else
            {
                ...
                return ProxyResponse.Create(status, headerList, Byte, cookieContainer.GetCookies(destinationUri), splitResponse[1]);
            }
        }
