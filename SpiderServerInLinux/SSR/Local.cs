        ...
        : IHandler
    {
        ...
        public int LogSocketException(Exception e)
        {
            ...
            else if (e is SocketException)
            {
                ...
                {
                    // closed by browser when sending normally happens when download is canceled or a
                    // tab is closed before page is loaded
                }
        ...
        private int endRemoteTCPRecv(IAsyncResult ar)
        {
            if (remote != null)
            {
                ...
                server.ServerSpeedLog().AddDownloadBytes(bytesRecv, DateTime.Now, true);
                if (sendback)
                {
        ...
        private int RemoteSend(byte[] bytes, int length)
        {
            ...
            if (send_len > 0)
            {
                ...
                server.ServerSpeedLog().AddUploadBytes(send_len, DateTime.Now, true);
                if (length >= 0)
                    ResetTimeout(cfg.TTL);
                else
