...
namespace Cowboy.WebSockets
{
    internal sealed class WebSocketServerHandshaker
    {
...
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("[{0}]{1}{2}", session.RemoteEndPoint, Environment.NewLine, request);
#endif
            try
            {
                // GET /chat HTTP/1.1
                ...
                // An HTTP/1.1 or higher GET request, including a "Request-URI" [RFC2616] that should
                // be interpreted as a /resource name/ defined in Section 3 (or an absolute
                // HTTP/HTTPS URI containing the /resource name/). A |Host| header field containing
                ...
                // A |Connection| header field that includes the token "Upgrade", treated as an ASCII
                ...
                // An |Upgrade| header field containing the value "websocket", treated as an ASCII
                // case-insensitive value.
                ...
                // A |Sec-WebSocket-Key| header field with a base64-encoded (see Section 4 of
                // [RFC4648]) value that, when decoded, is 16 bytes in length.
                ...
                // indicating which extensions the client would like to speak. The interpretation of
                // this header field is discussed in Section 9.1.
                if (extensions != null)
                {
                    if (!extensions.Any())
                        throw new WebSocketHandshakeException(string.Format(
                    ...
                    {
                        // The empty string is not the same as the null value for these purposes and
                ...
                // Optionally, an |Origin| header field. This header field is sent by all browser
                // clients. A connection attempt lacking this header field SHOULD NOT be interpreted
                // as coming from a browser client.
                //
                // Servers that are not intended to process input from any web page but only for
                // certain sites SHOULD verify the |Origin| field is an origin they expect. If the
                // origin indicated is unacceptable to the server, then it SHOULD respond to the
                // WebSocket handshake with a reply containing HTTP 403 Forbidden status code.
                //
                // The |Origin| header field protects from the attack cases when the untrusted party
                // is typically the author of a JavaScript application that is executing in the
                // context of the trusted client. The client itself can contact the server and, via
                // the mechanism of the |Origin| header field, determine whether to extend those
                // communication privileges to the JavaScript application. The intent is not to
                // prevent non-browsers from establishing connections but rather to ensure that
                // trusted browsers under the control of potentially malicious JavaScript cannot fake
                // a WebSocket handshake.

                // Optionally, other header fields, such as those used to send cookies or request
                // authentication to a server. Unknown header fields are ignored, as per [RFC2616].
            }
            catch (Exception ex)
            {
                InfoEvent.CowbotEvent.Message = string.Format("{0}{1}{2}", session, Environment.NewLine, request);
        ...
        internal static byte[] CreateOpenningHandshakeResponse(AsyncWebSocketSession session, string secWebSocketKey)
        {
            ...
            // A Status-Line with a 101 response code as per RFC 2616 [RFC2616]. Such a response
            // could look like "HTTP/1.1 101 Switching Protocols".
            ...
            // "258EAFA5- E914-47DA-95CA-C5AB0DC85B11", taking the SHA-1 hash of this concatenated
            // value to obtain a 20-byte value and base64- encoding (see Section 4 of [RFC4648]) this
            // 20-byte hash.
            ...
            // Optionally, a |Sec-WebSocket-Extensions| header field, with a value /extensions/ as
            // defined in step 4 in Section 4.2.2. If multiple extensions are to be used, they can
            // all be listed in a single |Sec-WebSocket-Extensions| header field or split between
            // multiple instances of the |Sec-WebSocket-Extensions| header field. A server accepts
            // one or more extensions by including a |Sec-WebSocket-Extensions| header field
            // containing one or more extensions that were requested by the client. The
            // interpretation of any extension parameters, and what constitutes a valid response by a
            // server to a requested set of parameters by a client, will be defined by each such extension.
            ...
            /**
                // Optionally, a |Sec-WebSocket-Protocol| header field, with a value /subprotocol/ as
                // defined in step 4 in Section 4.2.2.
                //
                // The client can request that the server use a specific subprotocol by including the
                // |Sec-WebSocket-Protocol| field in its handshake. If it is specified, the server
                // needs to include the same field and one of the selected subprotocol values in its
                // response for the connection to be established.
                //
                // These subprotocol names should be registered as per Section 11.5. To avoid
                // potential collisions, it is recommended to use names that contain the ASCII
                // version of the domain name of the subprotocol's originator. For example, if
                // Example Corporation were to create a Chat subprotocol to be implemented by many
                // servers around the Web, they could name it "chat.example.com". If the Example
                // Organization called their competing subprotocol "chat.example.org", then the two
                // subprotocols could be implemented by servers simultaneously, with the server
                // dynamically selecting which subprotocol to use based on the value sent by the client.
                //
                // Subprotocols can be versioned in backward-incompatible ways by changing the
                // subprotocol name, e.g., going from "bookings.example.net" to
                // "v2.bookings.example.net". These subprotocols would be considered completely
                // separate by WebSocket clients. Backward-compatible versioning can be implemented
                // by reusing the same subprotocol string but carefully designing the actual
                // subprotocol to support this kind of extensibility.
                */
            ...
            // Upgrade: websocket
            // Connection: Upgrade Sec-WebSocket-Accept: 1tGBmA9p0DQDgmFll6P0/UcVS/E=
            // Sec-WebSocket-Protocol: chat
            var response = sb.ToString();
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("[{0}]{1}{2}", session.RemoteEndPoint, Environment.NewLine, response);
#endif
            ...
            out List<string> protocols)
        {
            ...
            // The |Sec-WebSocket-Extensions| header field MAY appear multiple times in an HTTP
            // request (which is logically the same as a single |Sec-WebSocket-Extensions| header
            // field that contains all values. However, the |Sec-WebSocket-Extensions| header field
            // MUST NOT appear more than once in an HTTP response.
            extensions = null;
            // The |Sec-WebSocket-Protocol| header field MAY appear multiple times in an HTTP request
            // (which is logically the same as a single |Sec-WebSocket-Protocol| header field that
            // contains all values). However, the |Sec-WebSocket-Protocol| header field MUST NOT
