    ...
    public sealed class AsyncWebSocketSession : IDisposable
    {
        ...
        internal async Task Start()
        {
            ...
            try
            {
                ...
                InfoEvent.CowbotEvent.Message = string.Format("Session started for [{0}] on [{1}] in module [{2}] with session count [{3}].",
                    this.RemoteEndPoint,
            ...
            {
                InfoEvent.CowbotEvent.Message = string.Format(string.Format("Session [{0}] exception occurred, [{1}].", this, ex.Message), ex);
            ...
            var validateRemoteCertificate = new RemoteCertificateValidationCallback(
                ...
                =>
                {
                    ...
                    if (_configuration.SslPolicyErrorsBypassed)
                        ...
                        InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] error occurred when validating remote certificate: [{1}], [{2}].",
        ...
        private async Task HandleCloseFrame(Header frameHeader, byte[] payload, int payloadOffset, int payloadCount)
        {
...
#if DEBUG
                InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] received client side close frame [{1}] [{2}].", this, closeCode, closeReason);
#endif
            ...
            {
#if DEBUG
                InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] received client side close frame but no status code.", this);
#endif
...
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] received client side ping frame [{1}].", this, ping);
#endif
...
#if DEBUG
                InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] sends server side pong frame [{1}].", this, ping);
#endif
            }
        }
...
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] received client side pong frame [{1}].", this, pong);
#endif
                ...
                case _connected:
                    {
                        ...
                        try
                        {
                            ...
                            StartClosingTimer();
#if DEBUG
                            InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] sends server side close frame [{1}] [{2}].", this, closeCode, closeReason);
#endif
                        }
                        catch (Exception ex)
                        {
        ...
        private async Task InternalClose(bool shallNotifyUserSide)
        {
            ...
            if (shallNotifyUserSide)
            {
                try
                {
                    InfoEvent.CowbotEvent.Message = string.Format("Session closed for [{0}] on [{1}] in dispatcher [{2}] with session count [{3}].",
       this.RemoteEndPoint,
       ...
       this.Server.SessionCount - 1);
        ...
        {
            InfoEvent.CowbotEvent.Message = string.Format(string.Format("Session [{0}] error occurred in user side [{1}].", this, ex.Message), ex);
            await Task.CompletedTask;
        }
        ...
        {
            InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] keep-alive timer timeout [{1}].", this, KeepAliveTimeout);
...
#if DEBUG
                        InfoEvent.CowbotEvent.Message = string.Format("Session [{0}] sends server side ping frame [{1}].", this, string.Empty);
#endif
