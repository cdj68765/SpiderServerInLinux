...
namespace Cowboy.WebSockets
{
    internal sealed class WebSocketClientHandshaker
    {
        ...
        internal static byte[] CreateOpenningHandshakeRequest(AsyncWebSocketClient client, out string secWebSocketKey)
        {
            ...
            // The handshake MUST be a valid HTTP request as specified by [RFC2616]. The method of
            // the request MUST be GET, and the HTTP version MUST be at least 1.1. For example, if
            // the WebSocket URI is "ws://example.com/chat", the first line sent should be "GET /chat HTTP/1.1".
            ...
            // The request MUST include a header field with the name |Sec-WebSocket-Key|. The value
            // of this header field MUST be a nonce consisting of a randomly selected 16-byte value
            // that has been base64-encoded (see Section 4 of [RFC4648]). The nonce MUST be selected
            ...
            // The request MUST include a header field with the name |Sec-WebSocket-Version|. The
            ...
            // The request MAY include a header field with the name |Sec-WebSocket-Extensions|. If
            // present, this value indicates the protocol-level extension(s) the client wishes to
            // speak. The interpretation and format of this header field is described in Section 9.1.
            ...
            // wishes to speak, ordered by preference. The elements that comprise this value MUST be
            // non-empty strings with characters in the range U+0021 to U+007E not including
            // separator characters as defined in [RFC2616] and MUST all be unique strings. The ABNF
            // for the value of this header field is 1#token, where the definitions of constructs and
            ...
            // match the use-case described here for browser clients. The value of this header field
            // is the ASCII serialization of origin of the context in which the code establishing the
            // connection is running. See [RFC6454] for the details of how this header field value is constructed.

            // The request MAY include any other header fields, for example, cookies [RFC6265] and/or
            // authentication-related header fields such as the |Authorization| header field
            // [RFC2616], which are processed according to documents that define them.
...
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("[{0}]{1}{2}", client.RemoteEndPoint, Environment.NewLine, request);
#endif
        ...
        internal static bool VerifyOpenningHandshakeResponse(AsyncWebSocketClient client, byte[] buffer, int offset, int count, string secWebSocketKey)
        {
...
#if DEBUG
            InfoEvent.CowbotEvent.Message = string.Format("[{0}]{1}{2}", client.RemoteEndPoint, Environment.NewLine, response);
#endif
            try
            {
                ...
                // client using a 3xx status code (but clients are not required to follow them), etc.
                ...
                // If the response lacks a |Connection| header field or the |Connection| header field
                // doesn't contain a token that is an ASCII case-insensitive match for the value
                // "Upgrade", the client MUST _Fail the WebSocket Connection_.
                ...
                // concatenation of the |Sec-WebSocket- Key| (as a string, not base64-decoded) with
                // the string "258EAFA5- E914-47DA-95CA-C5AB0DC85B11" but ignoring any leading and
                ...
                // field indicates the use of an extension that was not present in the client's
                // handshake (the server has indicated an extension not requested by the client), the
                ...
                // field indicates the use of a subprotocol that was not present in the client's
                // handshake (the server has indicated a subprotocol not requested by the client),
                ...
                if (protocols != null)
                {
                    ...
                    // The value chosen MUST be derived from the client's handshake, specifically by
                    // selecting one of the values from the |Sec-WebSocket-Protocol| field that the
            ...
            {
                InfoEvent.CowbotEvent.Message = string.Format("{0}{1}{2}", client, Environment.NewLine, response);
            ...
            out List<string> protocols)
        {
            ...
            {
                // HTTP/1.1 101 Switching Protocols HTTP/1.1 400 Bad Request
